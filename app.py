...
# 1. Define States
t1_states = ["Scorer_In_T1", "Scorer_Out_T1", "NonScorer_In_T1", "NonScorer_Out_T1"] # Not Stressed
t2_states = ["Scorer_In_T2", "Scorer_Out_T2", "NonScorer_In_T2", "NonScorer_Out_T2"] # Stressed
transient_states = t1_states + t2_states + ["Shooting_Foul"]
...
# Initialize a uniform dummy matrix for the user to edit
default_P = np.zeros((num_t, len(all_states)))
for i in range(num_t):
    default_P[i, -1] = 1.0 # Default all to 3_pts just to prevent singular matrix on load
